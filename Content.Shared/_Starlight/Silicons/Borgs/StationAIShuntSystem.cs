...
public sealed partial class StationAIShuntSystem : EntitySystem
{
    ...
    private void OnAttemptShunt(EntityUid uid, StationAIShuntableComponent shuntable, AIShuntActionEvent ev)
    {
        ...
        if (_net.IsServer && HasComp<SiliconLawProviderComponent>(uid))
        {
            var getLaws = new GetSiliconLawsEvent(target);
            RaiseLocalEvent(target, ref getLaws);
            shunt.OldLawset = getLaws.Laws;

            var aiLaws = new GetSiliconLawsEvent(uid);
            RaiseLocalEvent(uid, ref aiLaws);
            _siliconLaw.SetLawset(target, aiLaws.Laws);
        }
