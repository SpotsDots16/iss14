...
public sealed partial class VocalSystem : EntitySystem
{
    ...
    public override void Initialize()
    {
        ...
        SubscribeLocalEvent<VocalComponent, SexChangedEvent>(OnSexChanged); // iss14: classic appearance raises this
    ...
    private void OnMapInit(EntityUid uid, VocalComponent component, MapInitEvent args)
    {
        ...
        // iss14: nothing raises VoiceChangedEvent with the classic humanoid appearance,
        // so load the species/sex default sounds here like pre-Nubody did.
    ...
    private void OnVoiceChanged(EntityUid uid, VocalComponent component, ref VoiceChangedEvent args) // iss14: by-ref to match [ByRefEvent]
    {
        LoadSounds(uid, component, args.NewVoice);
    }

    // iss14: reload the species/sex default sounds when the character's sex changes.
    ...
    {
        // iss14: no Nubody HumanoidProfileComponent here - derive the voice from the classic
        // humanoid appearance: the species' DefaultSoundsBySex indexed by the character's sex.
        if (protoId == null)
        {
            if (!TryComp<HumanoidAppearanceComponent>(uid, out var humanoid))
                return;

            if (!ProtoMan.TryIndex(humanoid.Species, out var species))
                return;

            var index = humanoid.Sex switch
            {
                Sex.Male => 0,
                Sex.Female => 1,
                _ => 2,
            ...
            if (index >= species.DefaultSoundsBySex.Length)
                return;

            protoId = species.DefaultSoundsBySex[index];
        }

        if (!ProtoMan.HasIndex(protoId.Value))
            return;
